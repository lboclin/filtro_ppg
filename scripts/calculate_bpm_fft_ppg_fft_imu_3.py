...
RESULTS_DIR = "results/fftPpg_com_valdidadorFftImu_3/"
...
def get_dominant_freq_and_power(data, fs, min_hz, max_hz):
    ...
    # Prepara o sinal para a FFT, agora apenas com detrend
    data_final = detrend(data)
    
    # A JANELA DE HANNING FOI REMOVIDA DESTA FUNÇÃO
...
if __name__ == "__main__":
    ...
    for ppg_filename in ppg_files:
        ...
        # Salva com um novo nome para não confundir com os resultados anteriores
        csv_path = os.path.join(RESULTS_DIR, f"{base_name}_bpm_results_final_v4.csv")
        results_df.to_csv(csv_path, index=False, float_format='%.2f')
        print(f"  ✅ Resultados de BPM finais (v4) salvos em: {csv_path}")
